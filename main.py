...
logger = logging.getLogger("MM_Video_Downloader")
...
async def download_callback(
    ...
    try:
        ...
        await query.edit_message_text(
            f"✅ <b>{quality_label}</b> "
            "Download ပြီးပါပြီ။\n\n"
            "📤 File ကို အပေါ်မှာ ပို့ပေးထားပါတယ်။",
            parse_mode="HTML"
        )
        ...
        if (
            ...
            await query.edit_message_text(
                ...
                "bot traffic လို့ သတ်မှတ်ထားပါတယ်။\n\n"
                "🔐 cookies.txt ကို Render Secret File "
                "ထဲမှာ မှန်မှန်ထည့်ထားကြောင်း စစ်ပါ။",
                parse_mode="HTML"
            )
        else:
            await query.edit_message_text(
                "❌ <b>Download Error</b>\n\n"
                "Video ကို download လုပ်လို့ မရပါဘူး။\n\n"
                "ခဏနေပြီး ပြန်စမ်းကြည့်ပါ။",
                parse_mode="HTML"
            )
        ...
        await query.edit_message_text(
            "❌ <b>Download လုပ်ရာမှာ Error ဖြစ်သွားပါတယ်။</b>\n\n"
